    ...
    protected void convert(BaseViewHolder holder, VideoItem item, int position, boolean isScrolling) {
        ...
        if (item.getEntity() == null) {
            ...
            if (!TextUtils.isEmpty(item.getUriString())) {
                Glide.with(mContext).load(item.getUriString()).placeholder(R.drawable.video_thumb).into(videoImage);
            } else if (!TextUtils.isEmpty(videoInfoEntity.getSnapshotUrl())) {
                Glide.with(mContext).load(videoInfoEntity.getSnapshotUrl()).placeholder(R.drawable.video_thumb).into(videoImage);
            } else {
